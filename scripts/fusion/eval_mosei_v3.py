#!/usr/bin/env python3
"""
Evaluate runs/mosei_6cls_v3/best_mosei_6cls.pt on val and test splits.
Usage (from repo root):
  python scripts/fusion/eval_mosei_v3.py
"""
import json
import sys
...
CKPT_PATH = "runs/mosei_6cls_v3/best_mosei_6cls.pt"
...
def report(split: str, probs: np.ndarray, y_cont: np.ndarray,
    ...
    # (0.5*sensitivity + 0.5*specificity), NOT support-weighted plain accuracy.
...
def main():
    ...
    cal_key = 'val_cal_macro_f1' if 'val_cal_macro_f1' in ckpt else 'val_macro_f1_calibrated'
    print(f"  val calib macro-F1   : {ckpt[cal_key]:.4f}")
