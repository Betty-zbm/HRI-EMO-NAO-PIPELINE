#!/usr/bin/env python3
"""
Evaluate an ensemble of checkpoints by averaging softmax probabilities.
Usage:
  python scripts/fusion/ensemble_eval.py  (uses CKPTS list below)
"""
...
CKPTS = [
    "runs/iemocap_fusion_seq_decoder_4cls/best_fusion_seq_decoder.pt",  # baseline
    "runs/iemocap_4cls_expA/best_fusion_seq_decoder.pt",                 # A
    "runs/iemocap_4cls_expB/best_fusion_seq_decoder.pt",                 # B
    "runs/iemocap_4cls_expC/best_fusion_seq_decoder.pt",                 # C
]

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Load all models
models, label2id = [], None
for path in CKPTS:
    ...
    print(f"Loaded {path}  (epoch={ckpt['epoch']}, d_model={args['d_model']}, seed={args['seed']})")
